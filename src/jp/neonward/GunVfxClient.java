...
public final class GunVfxClient implements ClientModInitializer {
 ...
 static boolean spraying(Minecraft mc,Player p){return p.isAlive()&&!p.isSpectator()&&p.isUsingItem()&&!GunReload.reloading(p.getUseItem())&&GunVfx.profile(p.getUseItem())==9&&CRYO.getOrDefault(p.getId(),0L)>mc.level.getGameTime()&&(p!=mc.player||mc.gui.screen()==null&&mc.options.keyAttack.isDown());}
 static final class Loop extends AbstractTickableSoundInstance {
  ...
  public void tick(){var mc=Minecraft.getInstance();if(mc.level!=level||mc.player==null||mc.isPaused()||!player.isAlive()||player.isRemoved()||player.distanceToSqr(mc.player)>24*24||!(charge?player.isUsingItem()&&!GunReload.reloading(player.getUseItem())&&GunVfx.profile(player.getUseItem())==6:spraying(mc,player))||(player==mc.player&&mc.gui.screen()!=null)){stop();return;}x=player.getX();y=player.getEyeY();z=player.getZ();pitch=charge?.7f+Math.min(1,player.getTicksUsingItem()/20f)*.5f:1;}
 }
 static void tick(Minecraft mc){if(mc.level!=world){clear();world=mc.level;}if(mc.level==null||mc.player==null)return;if(mc.isPaused()){for(var l:LOOPS.values())mc.getSoundManager().stop(l);LOOPS.clear();return;}budget=128;
  ...
  int players=0;for(var p:mc.level.players()){if(p.distanceToSqr(mc.player)>24*24||players++>=16)continue;boolean cryo=spraying(mc,p),charge=p.isUsingItem()&&!GunReload.reloading(p.getUseItem())&&GunVfx.profile(p.getUseItem())==6&&p.isAlive()&&(p!=mc.player||mc.gui.screen()==null);
