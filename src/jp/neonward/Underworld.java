...
public final class Underworld {
 ...
 static int request(ServerPlayer p,String action,int type){if(p.isSpectator()||StockMarket.ledger==null)return 0;String msg="追跡者は街の外だけ。指名手配中は外で隠れて解除。";
  ...
  var s=data(p);var o=new JsonObject();o.addProperty("street_ui","underworld");o.addProperty("summary","裏ランク "+s.rank()+" / 評判 "+s.reputation+" / 手配 "+s.heat+" / "+(s.mission<0?"依頼なし":"報酬 "+s.reward()+" Cr"));o.addProperty("objective",objective(s));o.addProperty("message",msg);ServerPlayNetworking.send(p,new StockMarket.Snapshot(o.toString()));return 1;
 }
