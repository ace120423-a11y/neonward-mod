...
public final class Cyberware {
 ...
 static boolean canEdit(ServerPlayer p,BlockPos pos){var b=p.level().getBlockState(pos).getBlock();boolean station=b==TERMINAL&&TerminalRoutes.roomAllowed(p.level(),pos,3);boolean personal=pos.equals(p.blockPosition());return !p.isSpectator()&&((p.distanceToSqr(Vec3.atCenterOf(pos))<=36&&station)||personal);}
 static boolean canSell(ServerPlayer p,BlockPos pos){return !p.isSpectator()&&p.level().getBlockState(pos).is(GuildServices.EXCHANGE)&&TerminalRoutes.roomAllowed(p.level(),pos,2)&&p.distanceToSqr(Vec3.atCenterOf(pos))<=36;}
