...
public final class GunControls {
 ...
 public static void init(){
  ClientTickEvents.START_CLIENT_TICK.register(mc->{if(!holding()){nextShot=0;return;}var gun=mc.player.getItemInHand(NeonArsenal.gunHand(mc.player));boolean click=false;while(mc.options.keyAttack.consumeClick())click=true;long now=System.currentTimeMillis();boolean wants=click||NeonArsenal.automatic(gun)&&mc.options.keyAttack.isDown();if(gun.is(NeonArsenal.ITEMS.get("cryo_projector")))wants=aiming()&&mc.options.keyAttack.isDown();if(wants&&now>=nextShot&&!mc.player.getCooldowns().isOnCooldown(gun)){mc.player.connection.sendCommand("neongun fire");nextShot=now+(NeonArsenal.automatic(gun)?320:160);}});
