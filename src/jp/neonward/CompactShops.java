...
public final class CompactShops {
 ...
 static void leave(ServerPlayer p,int id){var s=ALL[id];if(p.isPassenger())p.stopRiding();if(!p.teleportTo(p.level().getServer().overworld(),s.x()+1,65,s.z()+(id==4?-2.5:3.5),Set.of(),id==4?180:0,0,true))return;cooldown.put(p.getUUID(),p.level().getServer().getTickCount()+40);}
