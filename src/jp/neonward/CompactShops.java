...
public final class CompactShops {
 ...
 static boolean tryEnterFromStreet(ServerPlayer p){
  if(p.level().dimension()!=Level.OVERWORLD||p.getY()<64.5||p.getY()>=67||p.level().getServer().getTickCount()<cooldown.getOrDefault(p.getUUID(),0))return false;
  for(int id=0;id<ALL.length;id++){var a=ALL[id];if(p.getX()>a.x()-.15&&p.getX()<a.x()+2.15&&p.getZ()>a.z()-1&&p.getZ()<a.z()+.65&&p.level().getBlockState(new BlockPos(a.x(),65,a.z())).is(AutoDoors.DOOR))return enter(p,id);}
  return false;
 }
