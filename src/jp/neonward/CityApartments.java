...
public final class CityApartments {
 ...
 static boolean editable(Level l,Player p,BlockPos pos){if(!area(l,pos)||shaft(pos))return false;int f=floor(pos.getY());if(p instanceof ServerPlayer){if(!owns(p,f))return false;}else if(p==null||floor(p.getY())!=f)return false;int offset=pos.getY()-base(f);return pos.getX()>66&&pos.getX()<103&&pos.getZ()>217&&pos.getZ()<259&&offset>=0&&offset<=(f==20?6:5)&&!fixed.contains(new BlockPos(pos.getX(),offset,pos.getZ()));}
 static int available(MarketLedger l){for(int f=2;f<=19;f++){int n=f;if(l.accounts.values().stream().noneMatch(a->a.cityFloor==n))return f;}return 0;}
 static boolean vacant(MarketLedger l,int f){return f>=2&&f<=19&&l.accounts.values().stream().noneMatch(a->a.cityFloor==f);}
 static String buy(MarketLedger l,String who,int f){var a=l.account(who);if(a.cityFloor>0)return "購入済み / "+a.cityFloor+"階があなたの部屋です";if(f<2||f>19)throw new IllegalArgumentException("空いている2〜19階から選んでください");if(!vacant(l,f))throw new IllegalArgumentException("その階は売約済みです。別の空室を選んでください");if(a.cash<PRICE)throw new IllegalArgumentException("残高が足りません");a.cash-=PRICE;a.cityFloor=f;return f+"階を購入しました / 専用表札・家具配置対応";}
 static void init(){try(var r=new InputStreamReader(Objects.requireNonNull(CityApartments.class.getResourceAsStream("/data/neonward/housing/city_apartments.json")),java.nio.charset.StandardCharsets.UTF_8)){var o=JsonParser.parseReader(r).getAsJsonObject();founder=o.get("founder").getAsString();for(var e:o.getAsJsonArray("fixed")){var a=e.getAsJsonArray();fixed.add(new BlockPos(a.get(0).getAsInt(),a.get(1).getAsInt(),a.get(2).getAsInt()));}}catch(Exception e){throw new IllegalStateException("City apartment layout",e);}
  ServerLifecycleEvents.SERVER_STARTED.register(s->{if(StockMarket.ledger==null)return;var a=StockMarket.ledger.account(founder);if(a.cityFloor==0&&StockMarket.ledger.accounts.values().stream().noneMatch(v->v.cityFloor==20)){a.cityFloor=20;try{StockMarket.save();}catch(Exception e){a.cityFloor=0;}}});
  UseBlockCallback.EVENT.register((p,l,h,hit)->{if(!l.isClientSide()&&area(l,hit.getBlockPos())&&!shaft(hit.getBlockPos())&&!owns(p,floor(hit.getBlockPos().getY()))){p.sendOverlayMessage(Component.literal("この階は別の住人の部屋です"));return InteractionResult.FAIL;}return InteractionResult.PASS;});
  ServerTickEvents.END_SERVER_TICK.register(s->{if(StockMarket.ledger==null)return;var l=s.overworld();if(s.getTickCount()%10==0)for(var p:s.getPlayerList().getPlayers())if(!p.isSpectator()&&area(p.level(),p.blockPosition())&&!shaft(p.blockPosition())&&!owns(p,floor(p.getY()))){if(p.isPassenger())p.stopRiding();p.teleportTo(l,97.5,65,233.5,Set.of(),0,0,true);p.sendOverlayMessage(Component.literal("自分の階だけに入れます。不動産屋で購入してください"));}
