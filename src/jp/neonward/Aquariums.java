...
public final class Aquariums {
 ...
 static boolean fish(ItemStack s){return s.is(Items.COD)||s.is(Items.SALMON)||s.is(Items.TROPICAL_FISH)||s.is(Items.PUFFERFISH);}
 public static ItemStack caught(ItemStack s){if(!fish(s))return s;int length=10+java.util.concurrent.ThreadLocalRandom.current().nextInt(51);var n=new CompoundTag();n.putInt("neon_fish_cm",length);n.putString("neon_fish_id",UUID.randomUUID().toString());s.set(DataComponents.CUSTOM_DATA,CustomData.of(n));s.set(DataComponents.CUSTOM_NAME,Component.literal(s.getHoverName().getString()+" / "+length+" cm"));return s;}
 ...
 public static class Tank extends BlockEntity {
  NonNullList<ItemStack> items=NonNullList.withSize(1,ItemStack.EMPTY);String owner="";int clock;List<Display.BlockDisplay> mesh=new ArrayList<>();
  ...
  void clearDisplay(){for(var e:mesh)e.discard();mesh.clear();}
  ...
  void tick(){if(!(level instanceof ServerLevel server)||++clock%4!=0)return;if(items.getFirst().isEmpty()){clearDisplay();return;}if(mesh.isEmpty()||mesh.stream().anyMatch(e->!e.isAlive())){clearDisplay();String body=items.getFirst().is(Items.SALMON)?"red_terracotta":items.getFirst().is(Items.TROPICAL_FISH)?"orange_concrete":items.getFirst().is(Items.PUFFERFISH)?"yellow_terracotta":"light_gray_terracotta";
