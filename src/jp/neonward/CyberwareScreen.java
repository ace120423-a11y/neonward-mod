...
public class CyberwareScreen extends Screen {
 final BlockPos pos;int x,y,w,h,selected,family=0,tier=0;int[] counts=new int[CyberwareCatalog.PARTS.length],rolls=new int[CyberwareCatalog.PARTS.length];boolean requested,editable,sellable;MarketLedger.Account account=new MarketLedger.Account();String message="接続中…";
 ...
 @Override protected void init(){
  ...
  var sell=new PhoneScreen.NeonButton(x+112,y+h-25,95,18,"売却",b->request("sell",id));sell.active=sellable&&counts[id]>0;addRenderableWidget(sell);
 ...
 void receive(JsonObject o){if(!o.has("cyberware")||o.get("x").getAsInt()!=pos.getX()||o.get("y").getAsInt()!=pos.getY()||o.get("z").getAsInt()!=pos.getZ())return;editable=o.get("editable").getAsBoolean();sellable=o.has("sellable")&&o.get("sellable").getAsBoolean();if(o.has("rolls"))rolls=StockMarket.JSON.fromJson(o.get("rolls"),int[].class);if(o.has("inventory"))counts=StockMarket.JSON.fromJson(o.get("inventory"),int[].class);message=o.get("message").getAsString();if(o.has("account")){account=StockMarket.JSON.fromJson(o.get("account"),MarketLedger.Account.class);CyberwareCatalog.normalize(account);}rebuildWidgets();}
 @Override public void extractRenderState(GuiGraphicsExtractor g,int mx,int my,float delta){
  ...
  String status=font.plainSubstrByWidth(message,w-20);g.text(font,status,x+10,y+h-46,0xffff8b9e);g.centeredText(font,(editable?"CLINIC":sellable?"EXCHANGE":"閲覧")+" / HP +"+Math.round(b[0]*100)+"% / 防御 +"+Math.round(b[1]*100)+"%",cx,y+h-19,0xff7bc5c8);
