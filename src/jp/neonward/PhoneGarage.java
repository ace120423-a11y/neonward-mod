...
public final class PhoneGarage {
 ...
 public static void init(){
  CommandRegistrationCallback.EVENT.register((d,c,e)->{
   ...
   root.then(Commands.literal("view").executes(ctx->{var p=ctx.getSource().getPlayerOrException();var o=new com.google.gson.JsonObject();o.addProperty("garage",true);o.add("casinoVehicles",StockMarket.JSON.toJsonTree(StockMarket.ledger==null?java.util.Set.of():StockMarket.ledger.account(p.getStringUUID()).casinoVehicles));net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.send(p,new StockMarket.Snapshot(o.toString()));return 1;}));
 ...
 static int call(ServerPlayer p,VehicleCatalog.Spec spec){boolean bike=spec.bike();
  if(!CasinoGames.unlocked(StockMarket.ledger==null?null:StockMarket.ledger.account(p.getStringUUID()),spec.id()))return message(p,"この限定車はカジノの景品で入手できます。");
