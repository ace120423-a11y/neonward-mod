...
public final class PhonePhotoPicker extends Screen {
 ...
 @Override protected void init(){w=Math.min(330,width-16);h=Math.min(310,height-12);x=(width-w)/2;y=(height-h)/2;try{files=PhotoStore.photos();}catch(Exception e){message="写真を読み込めません";}index=Math.min(index,Math.max(0,files.size()-1));load();
  addRenderableWidget(new PhoneScreen.NeonButton(x+12,y+h-56,42,19,"<",b->{index=Math.max(0,index-1);load();}));addRenderableWidget(new PhoneScreen.NeonButton(x+60,y+h-56,42,19,">",b->{index=Math.min(Math.max(0,files.size()-1),index+1);load();}));addRenderableWidget(new PhoneScreen.NeonButton(x+110,y+h-56,w-122,19,"この写真を添付",b->{if(files.isEmpty())return;try{back.photo=encode(files.get(index));minecraft.gui.setScreen(back);back.request("view");}catch(Exception e){message="この写真は添付できません";}}));addRenderableWidget(new PhoneScreen.NeonButton(x+12,y+h-29,w-24,19,"戻る",b->onClose()));}
 ...
 @Override public void onClose(){minecraft.gui.setScreen(back);back.request("view");}
