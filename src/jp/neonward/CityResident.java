...
public class CityResident extends PathfinderMob {
 ...
 @Override public void tick(){super.tick();if(ShrineServices.freeze(this)||ExistingEateries.freeze(this))return;if(!level().isClientSide()&&(job.equals("fishing_buyer")||job.equals("fishing_rods")||job.startsWith("leisure_"))){setNoAi(true);setNoGravity(true);setDeltaMovement(Vec3.ZERO);setPos(home.getX()+.5,home.getY(),home.getZ()+.5);setYRot(job.startsWith("leisure_")?180:0);setYHeadRot(getYRot());yBodyRot=getYRot();return;}if(!level().isClientSide()&&tickCount%40==0&&home!=BlockPos.ZERO){boolean slum=kind().role()>=4&&kind().role()<10;if(shopStaff||kind().role()>=10){if(distanceToSqr(Vec3.atBottomCenterOf(home))>1)setPos(home.getX()+.5,home.getY(),home.getZ()+.5);getNavigation().stop();}else if(blockPosition().distSqr(home)>1600||!CityProtection.contains(level(),blockPosition())||CityProtection.southQuarter(blockPosition())!=slum||getY()<63){getNavigation().stop();setPos(home.getX()+.5,home.getY(),home.getZ()+.5);}}}
 @Override protected InteractionResult mobInteract(Player p,InteractionHand hand){
  if(hand==InteractionHand.MAIN_HAND&&p instanceof ServerPlayer visitor&&ShrineServices.use(visitor,this))return InteractionResult.SUCCESS;
  if(hand==InteractionHand.MAIN_HAND&&p instanceof ServerPlayer guest&&ExistingEateries.use(guest,this))return InteractionResult.SUCCESS;
  if(job.startsWith("leisure_")){if(hand==InteractionHand.MAIN_HAND&&p instanceof ServerPlayer sp)LeisureShop.use(sp,this);return InteractionResult.SUCCESS;}
