...
public final class ArsenalExpansion {
 ...
 public static void register(){
  ...
  ServerLifecycleEvents.SERVER_STOPPED.register(s->{VisibleBolts.clear();SHOTS.clear();ECHOES.clear();MELEE_NEXT.clear();});
  net.fabricmc.fabric.api.networking.v1.ServerPlayConnectionEvents.DISCONNECT.register((h,s)->{MELEE_NEXT.remove(h.player.getUUID());SHOTS.removeIf(v->{if(v.owner()!=h.player)return false;VisibleBolts.remove(v);return true;});ECHOES.removeIf(v->v.owner()==h.player);});
 }
 ...
 public static class Launcher extends NeonArsenal.Rifle {
  ...
  @Override InteractionResult fire(Level world,Player player,InteractionHand hand){if(!(player instanceof ServerPlayer p))return InteractionResult.SUCCESS;var gun=p.getItemInHand(hand);if(!p.isAlive()||p.isSpectator()||p.getCooldowns().isOnCooldown(gun))return InteractionResult.FAIL;
   if(type==3&&(!p.isUsingItem()||p.getUseItem()!=gun))return InteractionResult.FAIL;
 ...
 static void tick(net.minecraft.server.MinecraftServer server){VisibleBolts.clean();int now=server.getTickCount();for(var e:new ArrayList<>(ECHOES))if(now>=e.due()){ECHOES.remove(e);if(e.owner().isAlive()&&!e.owner().hasDisconnected()&&enemy(e.owner(),e.target())&&e.owner().distanceToSqr(e.target())<16&&e.owner().hasLineOfSight(e.target())){e.target().invulnerableTime=0;hurt(e.owner(),e.target(),e.damage());particles(e.owner().level(),e.owner().getEyePosition(),e.target().position().add(0,1,0),0xff83ce);}}
  for(var s:new ArrayList<>(SHOTS)){SHOTS.remove(s);var p=s.owner();if(!p.isAlive()||p.hasDisconnected()||p.level()!=s.level()||s.life()<=0||p.position().distanceToSqr(s.position())>160*160){VisibleBolts.remove(s);continue;}var l=p.level();var next=s.position().add(s.velocity());if(!l.hasChunkAt(BlockPos.containing(next))){VisibleBolts.remove(s);continue;}
   ...
   if(now%2==0)GunVfx.send(p,s.weapon(),2,s.mode(),s.position(),hit!=null?hit.getLocation():end);
   if(hit!=null){VisibleBolts.hit(s,hit.getLocation(),(LivingEntity)hit.getEntity());detonate(s,hit.getLocation(),(LivingEntity)hit.getEntity());}else if(wall.getType()!=HitResult.Type.MISS){VisibleBolts.hit(s,end,null);detonate(s,end.subtract(s.velocity().normalize().scale(.1)),null);}else {var step=new Shot(p,l,s.weapon(),next,s.velocity(),s.life()-1,s.damage(),s.mode());SHOTS.add(step);VisibleBolts.advance(s,step);}
  }
 }
}
