...
public final class NeonTelevision {
 ...
 static int set(ServerPlayer p,String input){
  ...
  if(p.level().getBlockEntity(origin(floor)) instanceof ScreenBlockEntity be){be.setURL(BlockSide.SOUTH,url);if(input.equals("off")){be.ytVolume=0f;var data=be.getScreen(BlockSide.SOUTH);if(data!=null){data.autoVolume=false;data.autoVolumeMaxLevel=0f;}}else tameVolume(be,BlockSide.SOUTH);be.setChanged();p.sendOverlayMessage(Component.literal(input.equals("off")?"テレビを停止しました":"テレビのページを切り替えました"));return 1;}return 0;
 }
}
