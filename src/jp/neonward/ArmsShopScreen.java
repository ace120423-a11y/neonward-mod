...
public class ArmsShopScreen extends Screen {
 ...
 protected void init(){w=Math.min(380,width-12);x=(width-w)/2;y=Math.max(4,(height-216)/2);for(int row=0;row<4;row++){int i=page*4+row;if(i>=NeonArsenal.DROPS.size())break;addRenderableWidget(new PhoneScreen.NeonButton(x+w-100,y+46+row*31,88,22,CompactShops.price(i)+" Cr 購入",b->request(i)));}int pages=(NeonArsenal.DROPS.size()+3)/4;addRenderableWidget(new PhoneScreen.NeonButton(x+12,y+176,40,18,"<",b->{page=(page+pages-1)%pages;rebuildWidgets();}));addRenderableWidget(new PhoneScreen.NeonButton(x+58,y+176,40,18,">",b->{page=(page+1)%pages;rebuildWidgets();}));addRenderableWidget(new PhoneScreen.NeonButton(x+w-100,y+176,88,18,"閉じる",b->onClose()));request(-1);}
