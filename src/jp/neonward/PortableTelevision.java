...
public final class PortableTelevision {
 ...
 else if(l.getBlockEntity(at) instanceof ScreenBlockEntity be){be.setURL(side,url);if(input.equals("off")){be.ytVolume=0f;var data=be.getScreen(side);if(data!=null){data.autoVolume=false;data.autoVolumeMaxLevel=0f;}}else tameVolume(be,side);be.setChanged();return 1;}p.sendOverlayMessage(Component.literal("テレビの上1ブロックを空けてください"));return 0;
 }
}
