...
public final class RealEstate {
 ...
 static int open(ServerPlayer p){if(!available(p))return 0;var req=new JsonObject();req.addProperty("app","8");req.addProperty("action","view");var o=PhoneServices.snapshot(StockMarket.ledger,p.getStringUUID(),req);o.addProperty("estate_ui",true);o.addProperty("action","view");o.addProperty("ok",true);o.addProperty("message","レイ：街の住宅は1フロア専有。個別住宅・農地も購入できます");ServerPlayNetworking.send(p,new StockMarket.Snapshot(o.toString()));return 1;}
 static void init(){
