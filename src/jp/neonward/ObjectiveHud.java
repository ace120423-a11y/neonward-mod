...
public final class ObjectiveHud {
 ...
 static void init(){try{var o=JsonParser.parseString(Files.readString(SETTINGS)).getAsJsonObject();visible=o.get("visible").getAsBoolean();configured=o.has("configured")&&o.get("configured").getAsBoolean();}catch(Exception ignored){}
  ...
  HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT,NeonWard.id("objectives"),(g,dt)->{var mc=Minecraft.getInstance();if(!visible||mc.player==null||mc.gui.screen()!=null)return;int w=Math.min(218,g.guiWidth()/2),x=g.guiWidth()-w-8,y=58;int capacity=Math.max(1,(g.guiHeight()-y-90)/40);int pages=Math.max(1,(rows.size()+capacity-1)/capacity),page=(ticks/160)%pages,start=page*capacity,count=Math.min(capacity,rows.size()-start),h=62+Math.max(1,count)*40;
