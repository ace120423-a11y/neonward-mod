"""Bounded offline Sakura visual QA. Copies the latest completed SakuraIntegration world.
...
for directory in root.glob('build/sakura-integration-*'):
    ...
    if 'SAKURA_TEST_COMPLETE' in text and 'SAKURA_TEST_FAILED' not in text and json.loads(marker.read_text(encoding='utf-8'))['phase']=='complete':candidates.append(directory)
...
for name in ('SakuraMaterials','SakuraTownPlan','SakuraTown','ShrineServices','ShrineScreen'):
    assert artifact.stat().st_mtime >= (root/f'src/jp/neonward/{name}.java').stat().st_mtime,f'Parent full build required: {name}'
test=root/('build/sakura-client-'+time.strftime('%Y%m%d-%H%M%S'))
...
with zipfile.ZipFile(test/'mods/neonward.jar') as jar:
    ...
    for name in ('SakuraMaterials','SakuraTownPlan','SakuraTown','ShrineServices','ShrineScreen'):assert f'jp/neonward/{name}.class' in jar.namelist()
...
(test/'SAKURA_VISUAL_ONLY').write_text('Isolated disposable world copy only\n',encoding='utf-8')
...
subprocess.run([str(jdk/'javac.exe'),'-proc:none','-encoding','UTF-8','-cp',compile_cp,'-d',str(test/'qa'),str(root/'tests/SakuraVisualQA.java')],check=True)
with zipfile.ZipFile(test/'mods/sakura-visual-qa.jar','w') as jar:
    jar.writestr('fabric.mod.json',json.dumps(dict(schemaVersion=1,id='sakura_visual_qa',version='1',environment='client',entrypoints=dict(client=['jp.neonward.SakuraVisualQA']))))
...
assert proc.returncode==0 and 'SAKURA_CLIENT_QA_COMPLETE captures=15' in text and 'SAKURA_CLIENT_QA_FAILED' not in text,'Sakura visual QA failed'
labels=re.findall(r'SAKURA_SCREENSHOT ([^\r\n]+)',text)
images=sorted((test/'screenshots').glob('*.png'),key=lambda p:p.stat().st_mtime_ns)
assert len(labels)==len(set(labels))==len(images)==15,'Missing/duplicate capture'
manifest=[dict(label=label,file=str(file)) for label,file in zip(labels,images)]
(test/'sakura-screenshots.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
print('SAKURA_CLIENT_PASS',test/'sakura-screenshots.json')
