...
qa_class='HousingSalesQA' if '--housing' in sys.argv else 'CasinoGachaQA' if gacha else 'ElementVfxQA' if vfx else 'ChainPullQA' if chain else 'WeaponMotionQA' if motion else 'PairedClientQA'
...
assert len(list((test/'screenshots').glob('*.png')))==(10 if vfx else 6 if chain or gacha else 4*len(focus.split(',')) if focus else 100 if motion else 4)
