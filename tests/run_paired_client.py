...
(test/'qa').mkdir()
subprocess.run([str(jdk/'javac.exe'),'-proc:none','-encoding','UTF-8','-cp',compile_cp,'-d',str(test/'qa'),str(root/'tests/PairedClientQA.java')],check=True)
with zipfile.ZipFile(test/'mods/paired-client-qa.jar','w') as z:
 z.writestr('fabric.mod.json',json.dumps(dict(schemaVersion=1,id='paired_client_qa',version='1',environment='client',entrypoints=dict(client=['jp.neonward.PairedClientQA']))))
...
assert len(list((test/'screenshots').glob('*.png')))==4
