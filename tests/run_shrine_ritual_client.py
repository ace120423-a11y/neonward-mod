"""Bounded offline shrine ritual animation QA. Copies the latest completed SakuraIntegration world.
...
for directory in root.glob('build/sakura-integration-*'):
    ...
    if 'SAKURA_TEST_COMPLETE' in text and 'SHRINE_RITUAL_SERVER_PASS' in text and 'SHRINE_BLESSING_COMBAT_PASS' in text and 'SAKURA_TEST_FAILED' not in text and json.loads(marker.read_text(encoding='utf-8'))['phase']=='complete':candidates.append(directory)
...
for name in ('SakuraMaterials','SakuraTownPlan','SakuraTown','ShrineServices','ShrineScreen','ShrineRituals','ShrineBlessings','ShrineRitualClient','ShrineRitualMeshes','ShrineRitualPose'):
    assert artifact.stat().st_mtime >= (root/f'src/jp/neonward/{name}.java').stat().st_mtime,f'Parent full build required: {name}'
test=root/('build/shrine-ritual-client-'+time.strftime('%Y%m%d-%H%M%S'))
...
with zipfile.ZipFile(test/'mods/neonward.jar') as jar:
    ...
    for name in ('SakuraMaterials','SakuraTownPlan','SakuraTown','ShrineServices','ShrineScreen','ShrineRituals','ShrineBlessings','ShrineRitualClient','ShrineRitualMeshes','ShrineRitualPose'):assert f'jp/neonward/{name}.class' in jar.namelist()
    mixins=json.loads(jar.read('neonward.mixins.json'))
    for name in ('ShrineRitualStateMixin','ShrineRitualExtractMixin','ShrineRitualHandsMixin','ShrineRitualBodyMixin','ShrineRitualPlayerMixin','ShrineRitualHeldMixin'):
        assert name in mixins['client'] and f'jp/neonward/mixin/{name}.class' in jar.namelist(),f'Missing render hook {name}'
...
(test/'SHRINE_RITUAL_VISUAL_ONLY').write_text('Isolated disposable world copy only\n',encoding='utf-8')
...
subprocess.run([str(jdk/'javac.exe'),'-proc:none','-encoding','UTF-8','-cp',compile_cp,'-d',str(test/'qa'),str(root/'tests/ShrineRitualVisualQA.java')],check=True)
with zipfile.ZipFile(test/'mods/shrine-ritual-visual-qa.jar','w') as jar:
    jar.writestr('fabric.mod.json',json.dumps(dict(schemaVersion=1,id='shrine_ritual_visual_qa',version='1',environment='client',entrypoints=dict(client=['jp.neonward.ShrineRitualVisualQA']))))
...
assert proc.returncode==0 and 'SHRINE_RITUAL_CLIENT_QA_COMPLETE captures=8' in text and 'SHRINE_RITUAL_CLIENT_QA_FAILED' not in text,'Sakura visual QA failed'
assert 'SHRINE_RITUAL_PACKET_CANCEL_PASS' in text and text.count('SHRINE_RITUAL_PACKET_STOP_PASS')==2,'Real packet start/completion/cancel synchronization not verified'
labels=re.findall(r'SHRINE_RITUAL_SCREENSHOT ([^\r\n]+)',text)
images=sorted((test/'screenshots').glob('*.png'),key=lambda p:p.stat().st_mtime_ns)
assert len(labels)==len(set(labels))==len(images)==8,'Missing/duplicate capture'
manifest=[dict(label=label,file=str(file)) for label,file in zip(labels,images)]
(test/'shrine-ritual-screenshots.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding='utf-8')
print('SHRINE_RITUAL_CLIENT_PASS',test/'shrine-ritual-screenshots.json')
